...
function injectLink(href: string, tag: string): HTMLLinkElement {
  ...
  link.rel = "stylesheet";
  link.href = href;
  link.dataset.facebookFonts = tag;
  document.head.appendChild(link);
  return link;
}

export default function FacebookGoogleFontsLoader() {
  useEffect(() => {
    if (!CRITICAL_FONTS_URL) return;
    if (document.querySelector('link[data-facebook-fonts="critical"]')) return;
    ...
    const addPreconnect = (href: string, crossOrigin?: string) => {
      ...
      el.dataset.facebookFonts = "preconnect";
    ...
    const loadDeferred = () => {
      ...
      if (document.querySelector('link[data-facebook-fonts="deferred"]')) return;
